...
////////=========////////////    EDITAR  ///////////=========////////////////////
/**
 * Metodo que permite mostrar los datos del subreceptor a editar
 */
if ($accion == "consultaEditar") {

    $id = $_POST['subreceptor'];

    $sql = "SELECT * FROM subreceptor WHERE idSubreceptor = $id";

    $consulta = $enlace->query($sql);
    $response = array();
    while ($sub = $consulta->fetch_assoc()) {
        $response = $sub;
    }
    echo json_encode($response);
}
/**
 * Metodo que permite editar el Subreceptor
 */

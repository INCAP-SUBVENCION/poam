...
<div id="omes4" class="table-responsive">
    <table class="table table-hover table-bordered" id="poa_omes_4" aria-describedby="">
	    ...
	    WHERE t1.subreceptor_id = $SUBRECEPTOR AND t1.periodo = 4 ORDER BY t1.estado";
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <?php } elseif ($periodo_4['estado'] == 'ES05') { ?>
                                        <li>
                                            <button class="dropdown-item" onclick="modalEditarPoa(<?php echo $SUBRECEPTOR; ?>, 4, <?php echo $periodo_4['idPoa']; ?>)">
                                                <em class="bi bi-pencil-square"></em> Editar </button>
                                        </li>
                                        <li>
                                            <button class="dropdown-item" onclick="modalCambiarEstadoPoa(<?php echo $periodo_4['idPoa']; ?>, <?php echo $ID; ?>, 'ES02' )">
                                                <em class="bi bi-arrow-clockwise"></em> Enviar correcciones </button>
                                        </li>
                                    <?php } ?>
                                </ul>
                            </div>
...
<!--- POM PPL ---->
<div id="ppl4" class="table-responsive">
    <table class="table table-hover table-bordered" id="poa_ppl_4" aria-describedby="">
        <thead class="text-center" style="font-size: 11px;">
            <th scope="">#</th>
            <th scope="">Mes</th>
            <th scope="">Municipio</th>
        ...
        <tfoot>
            ...
            <th scope="">Mes</th>
            <th scope="">Municipio</th>
            <td class="text-center"><strong id="pplnuevos4">0</strong></td>
            <td class="text-center"><strong id="pplrecurrentes4">0</strong></td>
            <td class="text-center"><strong id="ppltotal4">0</strong></td>
            <td class="text-center"><strong id="pplnatural4">0</strong></td>
            <td class="text-center"><strong id="ppllubricantes4">0</strong></td>
            <td class="text-center"><strong id="pplpruebavih4">0</strong></td>
            <td class="text-center"><strong id="pplreactivos4">0</strong></td>
            <td class="text-center"><strong id="pplsifilis4">0</strong></td>
            <th scope="">Observaciones</th>
            <th scope="">Estado</th>
        </tfoot>
    </table>
        ...
        <div class="col-sm-6">
            <form action="../../php/excel/generarExcelPoaOmes.php" method="POST">
                ...
                <select name="muni" id="muni">
                    ...
                    <?php
                    $sql = "SELECT DISTINCT p.municipio, c.nombre FROM poa p LEFT JOIN catalogo c ON c.codigo = p.municipio WHERE p.subreceptor_id = $SUBRECEPTOR AND p.periodo = 4;";
                    $rd = $enlace->query($sql);
                    while ($departamento = $rd->fetch_assoc()) { ?>
                        <option value="<?php echo $departamento['municipio'] ?>"><?php echo $departamento['nombre'] ?></option>
                    <?php } $rd->close(); ?>
                </select>
                <select name="me" id="me">
                    <option value="">Todo los meses ...</option>
                    <?php
                    $sql = "SELECT DISTINCT p.mes, c.nombre FROM poa p LEFT JOIN catalogo c ON c.codigo = p.mes WHERE p.subreceptor_id = $SUBRECEPTOR AND p.periodo = 4;";
                    $rd = $enlace->query($sql);
                    while ($departamento = $rd->fetch_assoc()) { ?>
                        <option value="<?php echo $departamento['mes'] ?>"><?php echo $departamento['nombre'] ?></option>
                    <?php } $rd->close(); ?>
                </select>

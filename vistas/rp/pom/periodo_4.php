...
<table class="table table-hover table-bordered" id="pom_periodo_4" aria-describedby="pom del periodo 1">
    ...
    <tbody style="font-size: 12px;">
        ...
        AND t2.estado NOT IN(SELECT estado FROM pom WHERE estado = 'ES01' OR estado = 'ES02')";
        if ($resp_1 = $enlace->query($sqlp_1)) {
            while ($periodo_1 = $resp_1->fetch_assoc()) { ?>
                <tr>
                    ...
                    <th scope><?php if ($periodo_1['estado'] == 'ES03') {
                                ...
                                ?>
                                    ...
                                    <li>
                                        <div class="d-grid gap-2">
                                            <button class="dropdown-item" onclick="modalCambiarEstadoPom(<?php echo $periodo_1['idPom']; ?>, <?php echo $ID; ?>, 'ES04')">
                                            <em class="bi bi-arrow-right-circle"></em> Revisar y aprobar  </button>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="d-grid gap-2">
                                            <button class="dropdown-item" onclick="enviarTodoPom(<?php echo $SUBRECEPTOR; ?>, 3, 'ES04', 'ES03')">
                                                <em class="bi bi-arrow-clockwise"></em> Revisar y aprobar todo </button>
                                        </div>
                                    </li>
                                <?php
                                }
